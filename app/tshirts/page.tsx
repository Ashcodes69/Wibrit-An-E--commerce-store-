      ...
      <div className="p-5">
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap mt-1">
            {Array.from(new Set(item.variants.map((v) => v.size))).map((size) => (
              <span
                key={size}
          ...
          <p className="text-sm font-medium text-gray-700 mt-2">Colors:</p>
          <div className="flex flex-wrap mt-1">
            {Array.from(new Set(item.variants.map((v) => v.color))).map((color) => (
              <span
                key={color}
